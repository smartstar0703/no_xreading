# main.py
# require pip install python-dotenv selenium pillow pytesseract webdriver-manager opencv-python beautifulsoup4

# Auto testing module
TestMode = False  # Trueだと問題も自動で解いてくれますが、あまり正確ではないです。
HasScript = True # driver.find_elements(By.XPATH, '//input[@type="button" and @value="Read Again"]')
# ^ これはあまり関係ないです。デフォルトのTrueを推奨します。
...
cid_match = re.search(r'cid=(\d+)', db_url)
if cid_match:
    print(f"Found: cid={cid_match.group(1)}")
    cid = f"cid={cid_match.group(1)}"

screenshot_dir = os.path.join("screenshots", cid)
text_output_path = f"script_{cid}.txt"
...
if HasScript:
    ...
    while True:
        ...
        next_wait = random.randint(150, 180) # ページをめくる秒数です。　早すぎるとFail扱いになります(n敗)
